...
const Proj = () => {
  ...
  const send = () => {
    ...
    const msgObj = { message, sender: user };
  ...
  function WriteAiMessage({ message }) {
    ...
    else if (typeof message === 'object') content = message.text || JSON.stringify(message);
    else content = String(message);
    ...
    axios
      ...
      .then((res) => {
        ...
        messageHandler = (data) => {
          ...
          const newMsg = { ...data, message: msgText };
  ...
  return (
    <main className="h-screen w-screen flex">
      <section className="left h-full min-w-100 flex flex-col bg-amber-100">
        ...
        {/* Chat Area */}
        <div className="flex-grow flex flex-col h-[calc(100vh-56px)]">
          <div
            ref={messageBox}
          ...
          >
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`message w-fit p-2 rounded-md flex flex-col break-words ${
                  msg.sender?._id === user._id
                    ? 'ml-auto bg-amber-200'
                    : msg.sender?._id === 'ai'
                    ? 'bg-purple-100 max-w-[70%]'
                    : 'bg-amber-300 max-w-[60%]'
                }`}
              >
          ...
          <div className="w-full bg-white p-3 border-t flex">
            <input
              type="text"
              value={message}
